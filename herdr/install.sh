#!/usr/bin/env bash
# herdr `[[build]]` step: download the prebuilt herdr-quotr binary for this platform from the
# matching GitHub Release into the plugin's bin/ dir. Runs on `herdr plugin install`; `herdr
# plugin link` skips the build step, so a local checkout builds from source instead — see the
# README's "Install (local)".
#
# The build runs with the plugin checkout as the working directory and may not receive the
# runtime env, so the plugin root is resolved from this script's location rather than from
# $HERDR_PLUGIN_ROOT. At runtime the pane command reads $HERDR_PLUGIN_ROOT/bin/herdr-quotr.
set -euo pipefail

NAME="herdr-quotr"
REPO="napalmpapalam/herdr-quotr"
